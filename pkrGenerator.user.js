...
async function createAndSetMap(inputText) {
    try {
        ...
        map.physics.fixtures = allShapes.map((r, i) => {
            ...
            if (r.isZoneIndicator) {
                fixture.n = 'zone_indicator #' + r.id;
            } else if (r.isZone) {
                fixture.n = 'cp_zone #' + r.id;
            } else if (r.isCapzone) {
                fixture.n = 'cp_real #' + r.id;
            } else if (r.isNoJump) {
        ...
        // Capzones
        allShapes.forEach((line, index) => { // <-- Added 'index' here
            if (line.isZone) {
                map.capZones.push({
                    n: line.id + '. cp_zone',
                    ty: 1,
                    l: 999, // Zones don't trigger game over
                    i: index, // <-- CRITICAL FIX: Use 'index' instead of 'line.id'
                });
            } else if (line.isCapzone) {
                map.capZones.push({
                    n: line.id + '. cp_real',
                    ty: 1,
                    l: 0.01,
                    i: index, // <-- CRITICAL FIX
                });
            }
...
function convertGameDataToJSON() {
    ...
    for (let i = 0; i < fixtures.length; i++) {
        ...
        let fixName = fixture.n || "";
        const isZone = fixName.includes('cp_zone');
        const isCapzone = fixName.includes('cp_real') || (capZoneMap.get(i) === 'capzone' && !isZone); // Fallback to handle old maps
        const isNoJump = capZoneMap.get(i) === 'nojump';
        const isZoneIndicator = fixName.includes('zone_indicator'); // <-- Recognize the tag
        ...
        const baseObject = {
            ...
            isBgLine: noPhysics,
            isZone: isZone,
            isZoneIndicator: isZoneIndicator // <-- Save it to the JSON
        };
