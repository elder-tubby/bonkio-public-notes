...
pkrEventScores.createWindow = function() {
    ...
    midControls.innerHTML = `
        <div style="display: grid; grid-template-columns: 85px 1fr 65px; align-items: center; width: 100%;">
            <div style="display: flex; align-items: center; gap: 4px;">
                <input type="checkbox" id="pkr_chat_alerts" style="cursor:pointer; margin: 0;" />
                <label for="pkr_chat_alerts" style="font-size: 11px; cursor:pointer; white-space: nowrap;">Chat Alerts</label>
            </div>
    ...
    midControls.querySelector("#pkr_chat_alerts").onchange = (e) => {
        this.state.chatAlerts = e.target.checked;
    };
            ...
            if (pData && !pData.top && !pData.eliminated) {
                if (isZone) {
                    if (!pData.zone) {
                        ...
                        if (this.state.chatAlerts) {
                            window.bonkAPI.chat(`${playerName} reached the zone on attempt ${pData.att_z}`);
                        }
                        this.renderTable();
                    }
                } else {
                    ...
                    if (this.state.chatAlerts) {
                        window.bonkAPI.chat(`${playerName} reached the capzone on attempt ${pData.att_t}`);
                    }
...
pkrEventScores.showRulesOverlay = function() {
    ...
    modal.innerHTML = `
        ...
        <button id="pkr_rules_close" style="margin-top: 15px; padding: 10px; background: #555; color: white; border: none; cursor: pointer; border-radius: 4px; font-weight: bold; font-family: 'futurept_b1'; font-size: 16px; flex-shrink: 0;">Got it!</button>
    `;
